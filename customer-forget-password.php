...
<?php
    if(isset($_POST['submit']))
    {
        try{
            ...
            $q = $conn->prepare("SELECT * FROM customer WHERE email=?");
            $q->execute([$_POST['email']]);
            $total = $q->rowCount();
            if(!$total) {
            throw new Exception("Email is not found");}
            $token = time();
            $statement = $conn->prepare("UPDATE customer SET token=? WHERE email=?");
            $statement->execute([$token,$_POST['email']]);
            $link = BASE_URL.'customer-reset-password.php?email='.$_POST['email'].'&token='.$token;
            ...
            try {
                ...
                $mail->send();
                $success_message = 'Please check your email and follo the steps.';
                } catch (Exception $e) {
